document.addEventListener("DOMContentLoaded",function(){
  ...
  if(feedbackForm){
    ...
    const endpoint=feedbackForm.dataset.endpoint||"";
    feedbackForm.addEventListener("submit",function(e){
      if(!endpoint){e.preventDefault();feedbackStatus.textContent="Formulář je připraven. Ještě je potřeba připojit ukládání podnětů.";feedbackStatus.className="feedback-status error";return;}
      feedbackForm.action=endpoint;feedbackStatus.textContent="Odesílám podnět…";feedbackStatus.className="feedback-status";
      window.setTimeout(function(){feedbackStatus.textContent="Děkujeme. Váš podnět byl odeslán.";feedbackStatus.className="feedback-status success";feedbackForm.reset();},900);
    });
    if(feedbackFrame){feedbackFrame.addEventListener("load",function(){if(feedbackForm.action&&feedbackForm.action!==window.location.href&&feedbackStatus.textContent==="Odesílám podnět…"){feedbackStatus.textContent="Děkujeme. Váš podnět byl odeslán.";feedbackStatus.className="feedback-status success";}});}
  }
});